...
export class ReferenceBookService {
  ...
  getDropdownData(typeEndpoint: string, endpoint: string): Observable<any[]> {
    const fullUrl = `${endpoint}`;

    switch (typeEndpoint.toLowerCase()) {
      case 'post':
        this.queryData.filters = this.queryData.filters || [];
        return this.http.post<any[]>(fullUrl, this.queryData, { headers: this.getHeader() });

      case 'get':
        // Для GET запросов передаем параметры через URL
        const params = this.queryData.filters ? this.convertToParams(this.queryData) : {};
      ...
      default:
        ...
        return this.http.get<any[]>(fullUrl, { headers: this.getHeader() });
    }
  }

  // Вспомогательный метод для конвертации фильтров в параметры URL
  private convertToParams(data: any): HttpParams {
    ...
    if (data.sortField) {
      params = params.set('sort', data.sortField);
      if (data.sortOrder) {
        params = params.set('order', data.sortOrder > 0 ? 'asc' : 'desc');
      }
    }
