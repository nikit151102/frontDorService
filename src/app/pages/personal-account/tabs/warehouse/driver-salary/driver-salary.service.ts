...
export class DriverSalaryService {
 ...
 getData(page: any = null, pageSize: any = null): Observable<any> {
  ...
  onFilterChange(filter: FilterDto) {
    ...
    this.getData(this.currentPage, this.pageSize).subscribe((data: any) => {
  ...
  loadProducts() {
    this.getData(this.counterpartyId).subscribe(
      (data) => {
        this.products = data.documentMetadata.data;
        this.totalInfo = data.totalInfo;
        this.setActiveData(data.documentMetadata.data)
  ...
  setSpend(data: any) {
    const token = localStorage.getItem('YXV0aFRva2Vu');
    let url = `${environment.apiUrl}/api/CommercialWork/DocInvoice/WarehouseDebit`;
