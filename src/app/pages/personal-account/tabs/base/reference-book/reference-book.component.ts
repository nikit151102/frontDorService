...
export class ReferenceBookComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.paramMap.subscribe(params => {
      const configId = params.get('configId'); // Получаем configId из URL
      this.typeId = configId; // Сохраняем в переменную компонента (если нужно)
      
      this.updateConfig(); // Обновляем конфигурацию
    });
